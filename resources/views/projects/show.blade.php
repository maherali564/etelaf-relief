@extends('layouts.app')
@section('title', trans_field($project, 'title'))
@section('og_image', $project->first_image ? asset('storage/'.$project->first_image) : '')
@section('content')
...
<section class="hero-detailed" @if($project->first_image) style="background-image:url('{{ asset('storage/'.$project->first_image) }}')" @endif>
    ...
    <div class="hero-detailed__inner">
        ...
        <h1 class="hero-detailed__title">{{ trans_field($project, 'title') }}</h1>
        @if($project->goal_amount > 0)
        <div class="hero-detailed__stats">
            <div class="hero-detailed__stat"><span class="hero-detailed__stat-value">${{ number_format($project->raised_amount ?? 0) }}</span><span class="hero-detailed__stat-label">{{ __('site.raised') }}</span></div>
            <div class="hero-detailed__stat"><span class="hero-detailed__stat-value">${{ number_format($project->goal_amount) }}</span><span class="hero-detailed__stat-label">{{ __('site.goal') }}</span></div>
            <div class="hero-detailed__stat"><span class="hero-detailed__stat-value">{{ $project->progressPercent() }}%</span><span class="hero-detailed__stat-label">{{ __('common.progress') }}</span></div>
        </div>
        <x-progress-bar :raised="$project->raised_amount ?? 0" :goal="$project->goal_amount" />
        @endif
    </div>
</section>
...
<section class="section">
    <div class="container">
        <div class="donation-layout">
            ...
            {{-- Right: Media Gallery + Project Card (60%) --}}
            <div class="gallery-container">
                ...
                <div class="project-detail-card">
                    <h2 class="project-detail-card__title">{{ trans_field($project, 'title') }}</h2>

                    <div class="project-detail-card__meta">
                        <div class="project-detail-card__meta-row">
                            <i class="fas fa-calendar-alt"></i>
                            <span>{{ __('common.published_date') ?? 'تاريخ النشر' }}: {{ $publishedDate }}</span>
                        </div>
                        <div class="project-detail-card__meta-row">
                            <i class="fas fa-map-marker-alt"></i>
                            <span>{{ __('common.location') ?? 'المكان' }}: {{ $projectLocation }}</span>
                        </div>
                    </div>

                    <div class="project-detail-card__desc">
                        {{ trans_field($project, 'description') }}
                    </div>

                    @php $shareUrl = urlencode(url()->current()); $shareText = urlencode(trans_field($project, 'title')); @endphp
...
@php $projectDonations = $donations ?? $project->donations()->completed()->latest()->limit(20)->get(); @endphp
...
<section class="section">
    <div class="container">
        <h2 class="section-title section-title--center">{{ __('common.similar_projects') ?? 'مشاريع مشابهة' }}</h2>
        <div class="similar-grid">
            @foreach($similar as $s)
            <a href="{{ route('projects.show', ['locale' => $currentLocale, 'slug' => $s->slug]) }}" class="similar-card">
                @if($s->first_image)
