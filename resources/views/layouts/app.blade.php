...
<head>
    ...
    <title>@yield('title', trans_field($s, 'site_name', $currentLocale) ?? config('app.name'))</title>
    <meta name="description" content="@yield('meta_description', trans_field($s, 'tagline', $currentLocale) ?? '')">
    <meta property="og:title" content="@yield('title', trans_field($s, 'site_name', $currentLocale) ?? config('app.name'))">
    <meta property="og:description" content="@yield('meta_description', trans_field($s, 'tagline', $currentLocale) ?? '')">
    <meta property="og:type" content="@yield('og_type', 'website')">
    <meta property="og:url" content="{{ url()->current() }}">
    <meta property="og:image" content="@yield('og_image', asset('storage/' . (($s->logos[$currentLocale] ?? $s->logo ?? ''))))">
...
<body class="{{ $isRtl ? 'rtl' : 'ltr' }} {{ $bodyClass ?? '' }}">
    <div id="devNotice" style="background:linear-gradient(135deg,#fbbf24,#f59e0b);color:#1c2e2b;text-align:center;padding:8px 16px;font-size:0.8rem;font-weight:600;position:sticky;top:0;z-index:10000;display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap">
        <span>{{ __('common.dev_notice') }}</span>
        <button onclick="localStorage.setItem('dev_notice_dismissed','1');this.parentElement.style.display='none'" style="padding:3px 12px;border:2px solid #1c2e2b;border-radius:999px;background:transparent;color:#1c2e2b;font-size:0.75rem;font-weight:700;cursor:pointer;white-space:nowrap">{{ __('common.dev_dismiss') }}</button>
    </div>
    ...
    <div id="cookieConsent" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:9999;padding:0 16px 16px;pointer-events:none" dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
        <div style="max-width:512px;margin:0 auto;background:rgba(9,9,11,0.95);-webkit-backdrop-filter:blur(24px);backdrop-filter:blur(24px);border:1px solid rgba(63,63,70,0.8);border-radius:24px;padding:20px 24px 20px;box-shadow:0 25px 60px rgba(0,0,0,0.5);pointer-events:auto;position:relative;overflow:hidden;transition:opacity 0.4s ease,transform 0.4s cubic-bezier(0.16,1,0.3,1);opacity:0;transform:translateY(30px)" id="cookieInner">
            ...
            <div style="display:flex;align-items:flex-start;gap:12px">
                ...
                <div style="flex:1;min-width:0">
                    <h4 style="margin:0 0 2px;font-size:14px;font-weight:800;color:#f4f4f5;letter-spacing:0.01em">{{ __('common.cookie_title') }}</h4>
                    <div id="cookieDesc" style="font-size:12px;color:#d4d4d8;font-weight:500;line-height:1.6">{{ __('common.cookie_desc') }}</div>
                </div>
            </div>
            <div id="cookiePanel" style="display:none;margin-top:12px;padding-top:12px;border-top:1px solid #18181b;max-height:260px;overflow-y:auto">
                <div style="display:flex;gap:12px;padding:12px;background:rgba(24,24,27,0.4);border:1px solid rgba(24,24,27,0.6);border-radius:12px;margin-bottom:10px;opacity:0.9;cursor:not-allowed">
                    ...
                    <div style="min-width:0"><strong style="font-size:12px;font-weight:700;color:#f4f4f5;display:block">{{ __('common.cookie_necessary') }}</strong><span style="font-size:11px;color:#a1a1aa;line-height:1.4;display:block">{{ __('common.cookie_necessary_desc') }}</span></div>
                </div>
                <div onclick="cookieToggle('analytics')" style="display:flex;gap:12px;padding:12px;border:1px solid;border-radius:12px;cursor:pointer;user-select:none;transition:all 0.2s;margin-bottom:10px" id="cookieAnalyticsRow">
                    ...
                    <div style="min-width:0"><strong style="font-size:12px;font-weight:700;color:#f4f4f5;display:block">{{ __('common.cookie_analytics') }}</strong><span style="font-size:11px;color:#a1a1aa;line-height:1.4;display:block">{{ __('common.cookie_analytics_desc') }}</span></div>
                </div>
                <div onclick="cookieToggle('marketing')" style="display:flex;gap:12px;padding:12px;border:1px solid;border-radius:12px;cursor:pointer;user-select:none;transition:all 0.2s" id="cookieMarketingRow">
                    ...
                    <div style="min-width:0"><strong style="font-size:12px;font-weight:700;color:#f4f4f5;display:block">{{ __('common.cookie_marketing') }}</strong><span style="font-size:11px;color:#a1a1aa;line-height:1.4;display:block">{{ __('common.cookie_marketing_desc') }}</span></div>
                </div>
            </div>
